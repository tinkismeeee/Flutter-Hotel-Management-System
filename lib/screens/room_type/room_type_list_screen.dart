...
class _RoomTypeListScreenState extends State<RoomTypeListScreen> {
  ...
  Future<void> goToAdd() async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const AddRoomTypeScreen()),
    );
    refreshData();
  }
  ...
  Future<void> goToEdit(RoomType roomType) async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => EditRoomTypeScreen(roomType: roomType)),
    );
    refreshData();
  }
  ...
  Future<void> confirmDelete(RoomType roomType) async {
    ...
    if (result == true) {
      final success = await RoomTypeService.deleteRoomType(roomType.roomTypeId);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            success ? 'Xóa loại phòng thành công' : 'Xóa loại phòng thất bại',
          ),
        ),
      );
